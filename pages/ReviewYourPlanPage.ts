...
export default class ReviewYourPlanPage {
    readonly    page: Page;
    readonly    dobField: Locator;
     ...
     async validateReviewYourPlanPageDisplayed() {
        // wait for network activity to quiet down
        await this.page.waitForLoadState('domcontentloaded').catch(() => {});
        await expect(this.page.getByRole('button', { name: 'Review your plan' })).toBeVisible({timeout: 80000});
        await expect(this.page.getByRole('heading', { name: 'Review your Scan.' })).toBeVisible({timeout: 80000});
     };
